...
export default class Community extends PIXI.Graphics {
    ...
    constructor(app: PIXI.Application, rect: PIXI.Rectangle, population: number) {
        super();
        ...
        for (let i = 0; i < population; i++) {
            const person = new Person(app, this.getRandomPoint(), Math.random() * 2 * Math.PI, params.personSpeed);
            ...
            for (let i = 0; i < this.people.length; i++) {
                ...
                if (person.infected) {
                    for (let j = 0; j < this.people.length; j++) {
                        ...
                        if (other.infected || other.status !== "alive" || i === j) continue;
    ...
    migrate(index: number, to: Community): void {
        if (index > this.people.length - 1 || this.people[index].status !== "alive") return;
